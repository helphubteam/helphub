...
RSpec.describe 'Api::V2::Login', type: :request do
  ...
  describe 'POST /api/v2/login' do
    context 'when password is correct' do
      ...
      it 'signs in volunteer successfully' do
        post api_v2_login_path, params: { email: email, password: password, app_version: new_app_version }
    ...
    context 'when password is wrong' do
      ...
      it 'responds unathorized error' do
        post api_v2_login_path, params: { email: email, password: wrong_password, app_version: new_app_version }
        expect(response).to have_http_status(403)
        expect(JSON.parse(response.body)).to eq({ 'errors' => [{ 'message' => 'Неверные логин или пароль' }] })
        expect(user.reload.app_version).to eq(old_app_version)
      end
  ...
  describe 'POST /api/v2/refresh_token' do
    context 'when not authroized' do
      it 'responds unathorized error' do
        post api_v2_refresh_token_path, params: { app_version: new_app_version }
