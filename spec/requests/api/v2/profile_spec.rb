...
RSpec.describe 'Api::V2::Profile', type: :request do
  ...
  describe 'GET /api/v2/profile' do
    it 'returns profile data' do
      get api_v2_profile_path
      expect(response).to have_http_status(200)
      expect(JSON.parse(response.body)).to eq(profile_response)
    end
  end

  describe 'POST /api/v2/subscribe' do
    let(:device_token) { '123qwe123qwe' }
    let(:device_platform) { 'android' }

    it 'setups device token data' do
      post api_v2_subscribe_path, params: { device_platform: device_platform, device_token: device_token }
      expect(response).to have_http_status(200)
  ...
  describe 'DELETE /api/v2/unsubscribe' do
    let(:user) do
    ...
    it 'drops device token data' do
      delete api_v2_unsubscribe_path
      expect(response).to have_http_status(200)
